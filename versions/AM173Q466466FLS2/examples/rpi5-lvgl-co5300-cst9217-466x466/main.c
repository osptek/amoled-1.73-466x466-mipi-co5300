 ...
 *
 * Modified for CO5300 DSI Screen (466x466) and Hynitron CST9217 Touch (/dev/input/event5)
 *
 ******************************************************************/
#include <unistd.h>
#include <pthread.h>
...
static void configure_simulator(int argc, char ** argv)
{
    ...
    /* 针对 466x466 CO5300 面板的默认分辨率配置 */
    settings.window_width = atoi(env_w ? env_w : "466");
    settings.window_height = atoi(env_h ? env_h : "466");
...
int main(int argc, char ** argv)
{
    ...
    /* 3. 配置 EVDEV 触摸设备节点为 Hynitron CST9217 的 /dev/input/event5 */
#if LV_USE_EVDEV
